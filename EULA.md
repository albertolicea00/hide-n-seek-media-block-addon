...
By installing, enabling, or using the **Hide & Seek** browser extension ("the Extension"), you ("the User") agree to be bound by this End User License Agreement ("EULA"). If you do not agree to these terms, you must not install or use the Extension.
...
Hide & Seek is a cross-browser extension for Google Chrome, Mozilla Firefox, and compatible Chromium-based browsers. It automatically blurs, blacks out, or replaces with placeholder images any media elements (images, videos, iframes) on websites the User visits, based on user-defined configuration. All processing occurs **entirely locally** within the User's browser.
...
The Extension collects **no personal data whatsoever**. All user settings (whitelist domains, blocking modes, custom CSS selectors, blur intensity, placeholder URLs) are stored exclusively in the browser's built-in `chrome.storage.sync` / `browser.storage.sync` API and are **never transmitted to any external server**.
...
- Content blocked by the Extension will always be appropriate or correctly classified.
